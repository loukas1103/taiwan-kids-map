...
# --- 2. 強化版地理定位功能 ---
def get_coordinates_advanced(address):
    # 預設座標：台北車站
    default_coords = (25.0478, 121.5170)
    
    if not address or address == "台北車站":
        return default_coords

    # 防呆：檢查是否直接輸入經緯度 (例如: 25.0, 121.5)
    coord_match = re.match(r'^[-+]?([1-8]?\d(\.\d+)?|90(\.0+)?),\s*[-+]?(180(\.0+)?|((1[0-7]\d)|([1-9]?\d))(\.\d+)?)$', address)
    if coord_match:
        try:
    ...
    # 嘗試進行地理編碼
    # 使用隨機 User-Agent 降低被阻擋機率
    u_agents = [f"my_travel_app_{random.randint(1,99999)}", f"taiwan_map_viewer_{random.randint(1,99999)}", "Mozilla/5.0"]
    
    for attempt in range(3):
        try:
            geolocator = Nominatim(user_agent=random.choice(u_agents))
            # 加上 Taiwan 縮小範圍，增加成功率
            loc = geolocator.geocode(f"{address}, Taiwan", timeout=10)
            if loc:
                return (loc.latitude, loc.longitude)
            # 如果找不到，嘗試不加 Taiwan 再找一次
    ...
    # 如果真的失敗，才回傳預設並顯示警告
    st.sidebar.warning("⚠️ 定位服務忙碌中，暫以台北車站為中心。您可以嘗試輸入更具體的名稱或直接輸入經緯度。")
    return default_coords
...
with st.sidebar.form("search_form"):
    target_address = st.text_input("1. 輸入您的中心位置", "台北車站", help="可輸入地址、地標名，或經緯度 (如: 25.04, 121.51)")
    
    TAIWAN_CITIES = ["全部縣市", "臺北市", "新北市", "桃園市", "臺中市", "臺南市", "高雄市", "新竹市", "新竹縣", "苗栗縣", "彰化縣", "南投縣", "雲林縣", "嘉義市", "嘉義縣", "屏東縣", "宜蘭縣", "花蓮縣", "臺東縣"]
    city_filter = st.selectbox("2. 選擇縣市", TAIWAN_CITIES)
...
# --- 4. 邏輯運算 ---
center_coords = get_coordinates_advanced(target_address)
...
# 顯示地圖
st_folium(m, width="100%", height=700, key="map_output")

# 顯示統計
if not filtered_df.empty:
    st.info(f"💡 在 {search_radius}km 範圍內找到了 {len(filtered_df)} 個景點！")
else:
    st.warning("⚠️ 該範圍內沒有找到景點，請嘗試放大搜尋半徑或切換中心點。")
